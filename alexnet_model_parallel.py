...
from model import alexnet_parallel
import time
...
model = alexnet_parallel().to_global(placement=PLACEMENT, sbp=B)
print(model)
model.train()
...
for t in range(EPOCH_NUM):
    ...
    for batch, (x, y) in enumerate(train_dataloader):
        x = x.to_global(placement=PLACEMENT, sbp=B)
        y = y.to_global(placement=PLACEMENT, sbp=B)
